...
class Seek extends Command {
    ...
    async run(message, args) {
        ...
        if (!args[0]) return message.channel.send("Please provide a number which is the duration (e.g. : 1:55) where will be the marker")

        let duration = args[0]

        if (!serverQueue.songs[0].info.isSeekable)
        return message.channel.send("Current track isn't seekable.");

   
    if (!this.durationPattern.test(duration))
        return message.channel.send("You provided an invalid duration (e.g.: 1:25)");

    const durationMs = this.durationToMillis(duration);
    if (durationMs > serverQueue.songs[0].info.length)
        return message.channel.send("The duration that you provided exceeds the duration of the current track.");

    try {
        await serverQueue.player.seek(durationMs);
        message.channel.send(`Seeked to ${duration}.`);
    } catch (e) {
